...
class LicenseRepository(BaseRepository[License]):
    ...
    async def update_license_state(self, license_id: str, new_state: LicenseState) -> License | None:
    ...
    async def update_license_plan(self, license_id: str, new_plan: LicensePlan) -> License | None:
        license = await self.get_by_license_id(license_id)
        if license:
    ...
    async def update_license_type(self, license_id: str, new_type: LicenseType) -> License | None:
        license = await self.get_by_license_id(license_id)
        if license:
    ...
    async def rotate_key(self, license_id: str, new_key: str, new_hash: str) -> License | None:
        license = await self.get_by_license_id(license_id)
        if license:
    ...
    async def update_expiration(self, license_id: str, new_expiration: datetime | None) -> License | None:
        license = await self.get_by_license_id(license_id)
        if license:
    ...
    async def get_all_by(self, state: LicenseState | None = None, type: LicenseType | None = None, tenant_id: str | None = None) -> list[License] | None:
